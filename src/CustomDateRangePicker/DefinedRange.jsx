...
class DefinedRange extends Component {
  ...
  handleRangeChange = (range, label) => {
    const { onChange, ranges, focusedRange, setRange } = this.props;
    ...
    if (label.includes("Today") || label.includes("Yesterday")) {
      ...
      setRange(7);
    } else if (label.includes("30") || label.includes("Month")) {
      setRange(30);
          ...
          {staticRanges.map((staticRange, i) => {
            ...
            return (
              <button
                ...
                style={{
                  color: selectedRange ? "white" || rangeColors[focusedRangeIndex] : null,
...
DefinedRange.defaultProps = {
  ...
  ranges: [],
  rangeColors: ["#E9650D", "#3ecf8e", "#fed14c"],
  focusedRange: [0, 0]
};
